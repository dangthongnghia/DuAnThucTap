...
export interface UICategory {
  id: string;
  title: string;
  icon: any; // Relaxed type for icon
  color: string;
}
...
interface CategorySelectorProps {
  ...
  suggestedCategory?: UICategory | null;
}
          ...
          {categories.map((category) => {
            ...
            return (
              <TouchableOpacity
                ...
                className={`items-center justify-center p-4 rounded-2xl border-2 min-w-[100px] ${isSelected
                    ? isDarkMode
                ...
                <Text
                  className={`text-sm font-semibold text-center ${isSelected
                      ? isDarkMode
                        ? 'text-violet-300'
                        : 'text-violet-700'
                      : isDarkMode
